...
## 15-01-2014
## Last modified by Barbara Bitarello: 18.05.2014
## Run NCV:
...
source("/mnt/sequencedb/PopGen/barbara/simulations/scripts/NCV.scanv7.r") 
...
if(FD==TRUE){
  ...
  for (i in ids) { # loop only for windows having at least one SNP
    if( nrow(input.list$INPUT.FD[[i]]) > 0) {
      NCV.scan4(INPUT.N=input.list$INPUT.NCV[[i]],FD=TRUE,FD.N=input.list$INPUT.FD[[i]],pop='YRI', WIN=input.list$WIN.POS[i,])->chNCV[[i]]
	} 
	if(nrow(input.list$INPUT.FD[[i]])<1) {
      NCV.scan4(INPUT.N=input.list$INPUT.NCV[[i]],FD=FALSE,pop='YRI',  WIN=input.list$WIN.POS[i,])->chNCV[[i]] # it does not work
    }
  }
assign(paste('res__',TAG, '_', BIN,'scanv7','_macaque',sep=''), do.call(rbind,chNCV))
}
if(FD==FALSE){
  input.list<-list(INPUT.NCV=chwinV2, WIN.POS=WIN.POS)
  for (i in ids){
    NCV.scan4(INPUT.N=input.list$INPUT.NCV[[i]], FD=FALSE,pop='YRI',  WIN=input.list$WIN.POS[i,])->chNCV[[i]]
  }
  assign(paste('res__',TAG, '_', BIN,'scanv7', '_macaque',sep=''), do.call(rbind,chNCV))  ##save NCV results
}
...
objectName<-paste('res__',TAG, '_', BIN, 'scanv7','_macaque',sep='')
