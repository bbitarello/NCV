...
#       Modified: 29.04.2014 by Barbara Bitarello
###############################################################################
...
NCV.scan4<-function(INPUT.N, pop='YRI',FD=T, FD.N) {  
  ...
  n<-100
  if(pop=='PUR'){n<-88}  #it would be ideal to run for all pops at once and dothe rest of the commands on a per pop basis.
	if(FD==TRUE){
...
#  if(nisnps>1){   #actually I don't need this because we only run the scan for windows with at least one SNP
	y2 <- as.data.frame(cbind(counts=as.numeric(INPUT.N[,pop])/n, pos=as.numeric(INPUT.N[,2]), ref=INPUT.N[,4], alt=INPUT.N[,5])) #alt allele freq
	...
	if(FD==T){
	real.snps <- which(y2[,counts] > 0) # which sites are polymorphic (eliminate cases where freq=0)
	...
	if(z$chimp[tmp[i]] == real.snps[real.snps$pos==temp.pos,alt]){  #if chimp in FD is == ALt in SNP, this is a fake FD, so exclude from FD
	...
	real.snps3<-real.snps2[which(real.snps2$counts!=0 & real.snps2$counts!=1),] #eliminate remaining snps with f=0 or f=1
...
########        #OUTPUT#	########	########
########	########	########	########
  final<- data.frame(Initial_seg_sites=nisnps, Initial_fds_sites=nifds, NCVf1=ncvf1,NCVf2=ncvf2, NCVf3=ncvf3,NCVf4=ncvf4, NCVf5=ncvf5,Nr.SNPs=polsites, Nr.FDs=fxdlen); 
  return(final);}
########################################################################################################################
