...
#############################################################################################################
#############################################################################################################
#must fix NCV.scanv2 to account correctly for FDs. Not trivial... But it works fine for my previous NCV.scan version.
...
#lapply(chwinV2, NCV.scan2, feq=0.4, snp_density=15)->chNCVd
...
#f4<-cbind(rep(NA, length(s)), rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)))
#f3<-cbind(rep(NA, length(s)), rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)))
#f2<-cbind(rep(NA, length(s)), rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)))
#f1<-cbind(rep(NA, length(s)), rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)),rep(NA, length(s)))
...
#Store(x22)
system.time(my.function(x1,tag='chr21')->res.test)
system.time(my.function(x1filt, tag='chr1')->res[[1]])
system.time(my.function(x2filt, tag='chr2')->res[[2]])
system.time(my.function(x3filt, tag='chr2')->res[[3]])
system.time(my.function(x3filt, tag='chr4')->res[[4]])
...
if (feq==0.1){
df<-cbind(res[[tag]][[1]],s)
df[,6]<-df2[,5]+W  #this dataframe won't have the SNPs because that takes time.
win<-as.data.frame(s[which(df[,2]<=w1)])
df2<-subset(df,df[,2]<=w1)}

if (feq==0.2){
df<-cbind(res[[tag]][[2]],s)
df[,6]<-df2[,5]+W  #this dataframe won't have the SNPs because that takes time.
win<-as.data.frame(s[which(df[,2]<=w2)])
df2<-subset(df,df[,2]<=w2)}

if (feq==0.3){
df<-cbind(res[[tag]][[3]],s)
df[,6]<-df2[,5]+W  #this dataframe won't have the SNPs because that takes time.
win<-as.data.frame(s[which(df[,2]<=w3)])
df2<-subset(df,df[,2]<=w3)}

if (feq==0.4){
df<-cbind(res[[tag]][[4]],s)
df[,6]<-df2[,5]+W  #this dataframe won't have the SNPs because that takes time.
win<-as.data.frame(s[which(df[,2]<=w4)])
df2<-subset(df,df[,2]<=w4)}

if (feq==0.5){
df<-cbind(res[[tag]][[5]],s)
df[,6]<-df2[,5]+W  #this dataframe won't have the SNPs because that takes time.
win<-as.data.frame(s[which(df[,2]<=w5)])
df2<-subset(df,df[,2]<=w5)}
...
l<-list(nam=nam,assign(paste('all_win', tag, sep='_',df),assign(paste('SNPs', 'chr',tag, sep='_'), df2))
return(l)
}
...
unlist(lapply(res$chr6$windows,function(x) dim(x)[1]))->temp
	...
	seq(length(temp)),
	log(temp),
	pch=20, ylab='Number of SNPs(log)', xlab='Window', ylim=c(0,max(log(temp))), main='Chr6 (YRI; 3kb window)', cex=0.3
	)
